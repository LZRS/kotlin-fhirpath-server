...
internal abstract class AbstractFhirPathService<Param : Any, Resource : Any> : FhirPathService {
  ...
  /** Version-specific FHIRPath evaluation engine. */
  protected abstract val fhirpathEngine: FhirPathEngine
  ...
  override suspend fun evaluate(inputData: InputData): JsonElement =
    withContext(Dispatchers.Default) {
      ...
      fun traceParameters(): List<Param> =
        fhirpathEngine.traces.map { entry ->
          makeStringParameter(
            name = "trace",
            value = entry.key,
            parts = entry.value.map { convertEvalResultToParameter(it.value, it.path) },
          )
        }
